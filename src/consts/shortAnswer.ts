export const shortAnswer = [
    ...
    {
        question: "简述基于Yield指标构建缺陷预测模型的步骤以及可能的改进方案。",
        answer: "步骤：\n" +
            "确定纳入影响因子的数据以及数据度量方法→收集并整理历史数据→依照回归技术进行计算→\n" +
            "项目进行过程中不断收集数据，与预测数据比较，调整回归参数→进行风险的识别、预防和控制\n" +
            "可能的改进方案：\n" +
            "假设注入水平和消除水平都符合正态分布，计算均值和标准差，可以用蒙特卡洛方法模拟结果。"
    },
    {
        question: "请简述DevOps的三个步骤",
        answer: "1. 实现开发到运维的工作快速地从左向右流动\n" +
    ...
    {
        question: "简述需求开发的完整过程，并解释客户需求和产品需求各自含义",
        answer: "需求开发过程：\n" +
            "需求获取、需求汇总、需求验证、需求文档制作\n" +
            "客户需求：描述客户的期望。往往表现为客户在实际工作中遇到一些具体问题，希望通过某个东西帮忙解决这些问题。\n" +
            "产品需求：描述开发团队针对客户需求提供的解决方案。"
    },
    {
        question: "请解释在质量保障活动中的V&V分别是什么含义？二者之间的关系是什么？",
        answer: "含义：\n" +
            "验证（Verification） & 确认（Validation）\n" +
            "验证是确保选定的工作产品与事先给定的需求一致；确认是确保开发完成的产品在即将使用该产品的环境中工作正确。\n" +
            "二者的关系：\n" +
            "验证与确认是相互依存的两个活动，都是为了提升最终产品质量而采取的措施。\n" +
            "验证的依据来源于确认的目标；验证活动为确认活动提供了前提条件。"
    },
    {
        question: "如何制定一份让人无法拒绝的计划？请描述剧本步骤和一些注意事项。",
        answer: "1. 制定任务计划，描述项目所有的任务清单，人物之间的先后顺序以及每个任务所需时间资源。\n" +
